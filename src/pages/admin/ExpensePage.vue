<template>
  <q-page padding class="w-full h-full max-w-[900px] mx-auto">
    <div>
      ...
      <q-scroll-area class="w-full" style="height:calc(100vh - 240px)">
        <q-list class="w-full">
          <q-slide-item :left-color="item.checked ? 'pink' : 'green'" right-color="blue"
            @left="props => onLeft(props, item)" @right="props => onRight(props, item)"
            v-for="(item, index) in expenses" :key="index" class="shadow-md my-2 rounded bg-gray-50">
            <template #left>
            ...
            <q-item class="w-full h-full">
              ...
              <q-item-section>
                ...
                <q-item-label caption>
                  <span v-if="item.repeat == 2">
                    {{ item.installment + ' / ' + item.installments }}
                  </span>
                  <span v-if="item.repeat == 1">
                    <q-icon name="1x_mobiledata" />
                  </span>
                  <span v-if="item.repeat == 3">
                ...
                <q-btn flat round color="red" icon="delete">
                  <q-menu>
                    <q-list separator class="bg-pink-100 text-pink-600 font-bold">
                      <q-item clickable v-close-popup @click="deleteExpense(item, 1)">
                        <q-item-section>{{ item.repeat == 1 ? 'Deletar Despesa' : 'Deletar Apenas Essa'
                          }}</q-item-section>
                      </q-item>
                      <q-item clickable v-close-popup @click="deleteExpense(item, 2)"
                        v-if="item.repeat == 2 || item.repeat == 3">
                        <q-item-section>Deletar Todas</q-item-section>
                      </q-item>
                      <q-item clickable v-close-popup @click="deleteExpense(item, 3)"
                        v-if="item.repeat == 2 || item.repeat == 3">
                        <q-item-section>
...
export default {
  ...
  computed: {
    expenses: {
      get() {
        return this.config.expenses
      },
      set(val) {
        this.config.expenses = val
      }
    },
    currentMonth: {
      get() {
